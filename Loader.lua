local MoonKeys = {"Moonhub77", "moonhub77"}
local KeyURL = "https://discord.gg/25HFrBRXEk"
...
ScreenGui.Name = "MoonHub_Ultimate_Auth"
...
local function StartExecution()
 ...
 if TargetPlace == 18687417158 or TargetPlace == 83645629621104 then
     local ExecSuccess, ExecError = pcall(function()
         loadstring(game:HttpGet("https://raw.githubusercontent.com/kingthekid074-hue/Cooking/refs/heads/main/Forsaken.lua"))()
     end)
...
KeyInput:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    if table.find(MoonKeys, InputText) then
    ...
    elseif #InputText > 0 and not table.find(MoonKeys, InputText) then
        StatusLabel.Text = "Validating..."
        StatusLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
        TweenService:Create(InputStroke, TweenInfo.new(0.3), {Color = Color3.fromRGB(80, 80, 100)}):Play()
        task.delay(0.4, function()
            if not table.find(MoonKeys, KeyInput.Text) and #KeyInput.Text > 0 then
